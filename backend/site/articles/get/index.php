...
if (isset($data["articles"]) && is_array($data["articles"])) {
    ...
    if ($article === null) {
        req_send(false, "Article with ID '$article' not found", 404); // HTTP code 404 : Not Found
    }

    // Favicon as base64
    if (isset($article["favicon"]) && filter_var($article["favicon"], FILTER_VALIDATE_URL) === false) {
        if (strpos($article["favicon"], "/") === false) {
            $file = "data/" . $article["id"] . "/" . $article["favicon"];
        } else {
    ...
    if (isset($article["banner"]) && filter_var($article["banner"], FILTER_VALIDATE_URL) === false) {
        if (strpos($article["banner"], "/") === false) {
            $file = "data/" . $article["id"] . "/" . $article["banner"];
        } else {
    ...
    $content_file = "data/" . $article["meta"]["id"] . "/source.md";
